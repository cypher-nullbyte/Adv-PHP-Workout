...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    // Validate user name
    if(empty($_POST["name"])){
        $nameErr = "<br>Please enter your name.";
    } else{
        $name = filterName($_POST["name"]);
        if($name == FALSE){
            $nameErr = "<br>Please enter a valid name.";
        }
    }
    ...
    if(empty($_POST["message"])){
        $messageErr = "<br>Please enter your comment.";     
    } else{
        $message = filterString($_POST["message"]);
        if($message == FALSE){
            $messageErr = "<br>Please enter a valid comment.";
        }
    }
...
<body style="text-align:center;padding-top:10vh;">
    <h2>Contact Us</h2>
